...
def format_label(
    text: str, label_type: str = "info", size: str | None = None, link: str | None = None, style: str | None = None
) -> str:
    classes = ["boost-label"]
    valid_types = [
        "success",
    ...
    if link and style:
        return format_html('<{} href="{}" class="{}" style="{}">{}</{}>', tag, link, css_class, style, text, tag)
    elif link:
        return format_html('<{} href="{}" class="{}">{}</{}>', tag, link, css_class, text, tag)
    elif style:
        return format_html('<{} class="{}" style="{}">{}</{}>', tag, css_class, style, text, tag)
    else:
...
def format_with_help_text(html_content: str, help_text: str | None = None) -> str:
    if help_text:
        return format_html("{}<br><small class=\"help\">{}</small>", html_content, help_text)
    return html_content
